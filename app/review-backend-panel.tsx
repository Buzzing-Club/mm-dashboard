...
export function ReviewBackendPanel({data}:{data:BackendReview}) {
  return <div className="review-backend-panel">
    ...
    <div className="review-seven-table-wrap"><table className="review-seven-table"><caption>阶段 PnL 与库存估值 · USDB · 近似值</caption><thead><tr><th>阶段</th><th title="阶段内现金流加持仓估值变动；不是已实现收益，也不与点差/敞口归因相加。">阶段 PnL</th><th>累计 PnL</th><th>未实现 PnL</th><th>YES / NO (sh)</th><th>参考 YES 价格</th><th>价格时间 (UTC+8)</th></tr></thead><tbody>{data.valuations?.map(row=><tr key={row.phase} title={`${row.markSource}；${row.reason}；估值截止 ${row.at}`}><th>{row.phase}{!row.phaseComplete?' · 未结束':''}</th><td>{money(row.phasePnl)}</td><td>{money(row.cumulativePnl)}</td><td>{money(row.unrealized)}</td><td>{row.yes===null?'--':`${row.yes.toFixed(3)} / ${row.no?.toFixed(3)}`}</td><td>{row.markYes===null?'--':row.markYes.toFixed(4)}</td><td>{row.markAt?timestamp(row.markAt):row.markSource}</td></tr>)}</tbody></table></div>
