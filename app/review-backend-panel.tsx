...
const money = (value: number | null) => value === null ? '无完整数据' : `${value < 0 ? '-' : ''}$${formatReviewNumber(Math.abs(value))}`;
...
export function ReviewBackendPanel({data}:{data:BackendReview}) {
  return <div className="review-backend-panel">
    ...
    <div className="review-seven-table-wrap"><table className="review-seven-table"><caption>阶段 PnL 与库存估值 · USDB · 近似值</caption><thead><tr><th>阶段</th><th title="阶段内现金流加持仓估值变动；不是已实现收益，也不与点差/敞口归因相加。">阶段 PnL</th><th>累计 PnL</th><th>未实现 PnL</th><th>YES / NO (sh)</th><th>参考 YES 价格</th><th>价格时间 (UTC+8)</th></tr></thead><tbody>{data.valuations?.map(row=><tr key={row.phase} title={`${row.markSource}；${row.reason}；估值截止 ${row.at}`}><th>{row.phase}{!row.phaseComplete?' · 未结束':''}</th><td>{money(row.phasePnl)}</td><td>{money(row.cumulativePnl)}</td><td>{money(row.unrealized)}</td><td>{row.yes===null?'--':`${formatReviewNumber(row.yes)} / ${row.no == null ? '--' : formatReviewNumber(row.no)}`}</td><td>{row.markYes===null?'--':formatReviewNumber(row.markYes)}</td><td>{row.markAt?timestamp(row.markAt):row.markSource}</td></tr>)}</tbody></table></div>
    ...
    {data.lots.length > 0 && <div className="review-seven-table-wrap review-pnl-lots" role="region" aria-label="已处置敞口批次" tabIndex={0}><table className="review-seven-table"><caption>已处置敞口批次 · FIFO · 最多展示 500 行</caption><thead><tr><th>建仓阶段</th><th>Outcome</th><th>数量 (sh)</th><th>建仓均价</th><th>处置 / 结算价</th><th>毛收益</th></tr></thead><tbody>{data.lots.map((row,i)=><tr key={i}><th>{row.phase}</th><td>{row.outcome}</td><td>{formatReviewNumber(row.quantity)}</td><td>{formatReviewNumber(row.entryPrice)}</td><td>{formatReviewNumber(row.exitPrice)}</td><td>{money(row.pnl)}</td></tr>)}</tbody></table></div>}
