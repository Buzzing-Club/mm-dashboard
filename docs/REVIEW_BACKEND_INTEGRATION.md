...
1. 全市场逐阶段点差/敞口/总 PnL 的完整汇总，以及未处置库存的历史阶段末估值，尚未实现。本次新增的是选中市场的有界事实归因；已有全市场当前账户 PnL 汇总保持不变。大市场需要分页复盘任务或持久化有界聚合，不能简单提高当前请求上限。
2. MM 与 TradeVolume 执行角色：backend self/internal/user 是账户分类，不等于策略角色。当前账本面板明确显示 Key 账户范围；指标若需纯 MM，仍须关联策略订单角色，不能用 internal 代替。
3. 开盘毒性、端到端报价/跟随延迟、NORMAL 双边盘口持续时间、60 秒公允价回摆、完整计划计数：应接策略观测/确认链路。新的 backend fills 不包含这些策略事实，不能用下一成交价、挂撤单延迟或采样笔数偷换口径。
